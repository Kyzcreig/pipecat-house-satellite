...
// pre-change baselines. Ack-only codec controls report null.
static esp_err_t xvf_read_handler(httpd_req_t *req) {
  char query[64] = {0};
  char param[32] = {0};
  if (httpd_req_get_url_query_str(req, query, sizeof(query)) != ESP_OK ||
      httpd_query_key_value(query, "param", param, sizeof(param)) != ESP_OK) {
    httpd_resp_set_status(req, "400 Bad Request");
    return httpd_resp_sendstr(req, "{\"error\":\"need ?param=<name>\"}");
  }
  ...
  if (ret == ESP_OK) {
    if (readback_valid) {
      snprintf(body, sizeof(body),
               "{\"ok\":true,\"param\":\"%s\",\"readback\":%.9g}", param,
               (double)readback);
    } else {
      snprintf(body, sizeof(body),
               "{\"ok\":true,\"param\":\"%s\",\"readback\":null}", param);
    }
    httpd_resp_set_type(req, "application/json");
    return httpd_resp_sendstr(req, body);
  }
  if (ret == ESP_ERR_NOT_FOUND) {
    ...
    httpd_resp_set_status(req, "500 Internal Server Error");
    snprintf(body, sizeof(body), "{\"error\":\"read failed: %s\"}",
             esp_err_to_name(ret));
  }
  return httpd_resp_sendstr(req, body);
}
