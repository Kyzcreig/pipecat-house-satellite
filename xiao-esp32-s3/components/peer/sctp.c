...
int sctp_outgoing_data(Sctp* sctp, char* buf, size_t len, SctpDataPpid ppid, uint16_t sid) {
  ...
  chunk->iube = 0x06;
  chunk->sid = htons(sid);
  chunk->sqn = htons(ppid == DATA_CHANNEL_PPID_CONTROL ? 0 : sqn++);
...
void sctp_add_stream_mapping(Sctp* sctp, const char* label, uint16_t sid,
  ...
  for (int i = 0; i < sctp->stream_count; i++) {
    SctpStreamEntry* existing = &sctp->stream_table[i];
    int same_sid = existing->sid == sid;
    int same_label =
        strncmp(existing->label, label, sizeof(existing->label)) == 0;
    if (same_sid && same_label) {
      existing->channel_type = channel_type;
      existing->reliability_parameter = reliability_parameter;
      return;
    }
    if (same_sid || same_label) {
      LOGE("Rejecting conflicting SCTP stream mapping");
      return;
    }
  }
  if (sctp->stream_count < SCTP_MAX_STREAMS) {
    SctpStreamEntry* stream = &sctp->stream_table[sctp->stream_count];
    strncpy(stream->label, label, sizeof(stream->label) - 1);
...
int sctp_parse_data_channel_open(Sctp* sctp, uint16_t sid, char* data,
                                 size_t length, int send_ack) {
  if (length < 12 || data == NULL)
    return 0;  // Not enough data for a DATA_CHANNEL_OPEN message

  if (data[0] == DATA_CHANNEL_OPEN) {
    uint8_t channel_type = (uint8_t)data[1];
    uint32_t reliability_parameter =
        ((uint32_t)(uint8_t)data[4] << 24) |
        ((uint32_t)(uint8_t)data[5] << 16) |
        ((uint32_t)(uint8_t)data[6] << 8) |
        (uint8_t)data[7];
    uint16_t label_length =
        (uint16_t)(((uint16_t)(uint8_t)data[8] << 8) | (uint8_t)data[9]);
    uint16_t protocol_length =
        (uint16_t)(((uint16_t)(uint8_t)data[10] << 8) | (uint8_t)data[11]);

    // Strict bounds: stream labels live in a fixed 32-byte table entry.
    if (label_length == 0 || label_length >= sizeof(sctp->stream_table[0].label) ||
        (size_t)label_length + (size_t)protocol_length != length - 12)
      return 0;

    char* label = (char*)(data + 12);

    // copy and null-terminate
    char label_str[sizeof(sctp->stream_table[0].label)];
    ...
    // Add stream mapping
    sctp_add_stream_mapping(sctp, label_str, sid, channel_type,
...
#if !CONFIG_USE_USRSCTP
static void sctp_set_inbound_initial_tsn(Sctp* sctp, uint32_t initial_tsn) {
  sctp->inbound_cumulative_tsn = initial_tsn - 1;
  sctp->inbound_gap_bitmap = 0;
  sctp->inbound_tsn_valid = 1;
}

static int sctp_record_inbound_tsn(Sctp* sctp, uint32_t tsn) {
  if (!sctp->inbound_tsn_valid) {
    sctp_set_inbound_initial_tsn(sctp, tsn);
  }
  int32_t delta = (int32_t)(tsn - sctp->inbound_cumulative_tsn);
  if (delta <= 0) {
    return 0;  // duplicate or already covered by FORWARD-TSN
  }
  if (delta > 64) {
    LOGW("SCTP TSN gap exceeds 64-packet receive window");
    return 0;
  }
  sctp->inbound_gap_bitmap |= UINT64_C(1) << (delta - 1);
  while (sctp->inbound_gap_bitmap & UINT64_C(1)) {
    sctp->inbound_cumulative_tsn++;
    sctp->inbound_gap_bitmap >>= 1;
  }
  return 1;
}

static void sctp_record_forward_tsn(Sctp* sctp, uint32_t new_cumulative_tsn) {
  if (!sctp->inbound_tsn_valid) {
    sctp_set_inbound_initial_tsn(sctp, new_cumulative_tsn + 1);
    return;
  }
  int32_t delta = (int32_t)(new_cumulative_tsn - sctp->inbound_cumulative_tsn);
  if (delta <= 0) {
    return;
  }
  sctp->inbound_cumulative_tsn = new_cumulative_tsn;
  sctp->inbound_gap_bitmap =
      delta >= 64 ? 0 : sctp->inbound_gap_bitmap >> delta;
  while (sctp->inbound_gap_bitmap & UINT64_C(1)) {
    sctp->inbound_cumulative_tsn++;
    sctp->inbound_gap_bitmap >>= 1;
  }
...
static size_t sctp_fill_sack(Sctp* sctp, SctpSackChunk* sack) {
  sack->common.type = SCTP_SACK;
  sack->common.flags = 0;
  sack->cumulative_tsn_ack = htonl(sctp->inbound_cumulative_tsn);
  sack->a_rwnd = htonl(0x100000);
  ...
  int offset = 1;
  while (offset <= 64) {
    if (!(sctp->inbound_gap_bitmap & (UINT64_C(1) << (offset - 1)))) {
      offset++;
      continue;
    }
    int start = offset;
    while (offset <= 64 &&
           (sctp->inbound_gap_bitmap & (UINT64_C(1) << (offset - 1)))) {
      offset++;
    }
    blocks[block_count * 2] = htons((uint16_t)start);
    blocks[block_count * 2 + 1] = htons((uint16_t)(offset - 1));
    block_count++;
  }
  sack->number_of_gap_ack_blocks = htons(block_count);
  size_t sack_length = sizeof(SctpSackChunk) + block_count * 4;
  sack->common.length = htons((uint16_t)sack_length);
  return sack_length;
}

static size_t sctp_write_prsctp_params(uint8_t* out) {
  SctpChunkParam* pr = (SctpChunkParam*)out;
  pr->type = htons(SCTP_PARAM_PRSCTP_SUPPORTED);
  pr->length = htons(4);

  SctpChunkParam* supported = (SctpChunkParam*)(out + 4);
  supported->type = htons(SCTP_PARAM_SUPPORTED_CHUNKS);
  supported->length = htons(5);
  supported->value[0] = SCTP_FORWARD_TSN;
  memset(out + 9, 0, 3);  // parameter padding to a 4-byte boundary
  return 12;
}
#endif
...
void sctp_handle_sctp_packet(Sctp* sctp, char* buf, size_t len) {
  ...
  if (ppid == DATA_CHANNEL_PPID_CONTROL)
    sctp_parse_data_channel_open(sctp, sid, buf + 28, len - 28, 1);
}
...
void sctp_incoming_data(Sctp* sctp, char* buf, size_t len) {
  ...
  while ((4 * (pos + 3) / 4) < len) {
    ...
    uint16_t chunk_length = ntohs(chunk_common->length);
    if (chunk_length < sizeof(SctpChunkCommon) || pos + chunk_length > len) {
      LOGE("invalid SCTP chunk length %u", (unsigned)chunk_length);
      return;
    }
    ...
    switch (chunk_common->type) {
      case SCTP_DATA: {
        ...
        if (chunk_length < sizeof(SctpDataChunk) + 1) {
          LOGE("short SCTP DATA chunk");
          return;
        }

        int is_new =
            sctp_record_inbound_tsn(sctp, ntohl(data_chunk->tsn));
        size_t sack_length = sctp_fill_sack(sctp, sack_chunk);
        length = sack_length + sizeof(SctpHeader);
        ...
        if (ntohl(data_chunk->ppid) == DATA_CHANNEL_PPID_CONTROL &&
          ...
          if (sctp_parse_data_channel_open(
                  sctp, ntohs(data_chunk->sid), (char*)data_chunk->data,
                  chunk_length - sizeof(SctpDataChunk), 0)) {
            SctpDataChunk* ack_chunk =
                (SctpDataChunk*)((uint8_t*)sack_chunk + sack_length);
            ack_chunk->type = SCTP_DATA;
            ack_chunk->iube = 0x03;
            ack_chunk->tsn = htonl(sctp->tsn++);
            ack_chunk->sid = data_chunk->sid;
            ack_chunk->sqn = htons(0);
            ack_chunk->ppid = htonl(DATA_CHANNEL_PPID_CONTROL);
            ack_chunk->length = htons(1 + sizeof(SctpDataChunk));
            ack_chunk->data[0] = DATA_CHANNEL_ACK;
            length += 4 * ((ntohs(ack_chunk->length) + 3) / 4);
          }
        } else if (is_new &&
                   (ntohl(data_chunk->ppid) == DATA_CHANNEL_PPID_DOMSTRING ||
      ...
      case SCTP_FORWARD_TSN: {
        if (chunk_length < sizeof(SctpChunkCommon) + sizeof(uint32_t)) {
          LOGE("short SCTP FORWARD-TSN chunk");
          return;
        }
        SctpForwardTsnChunk* forward = (SctpForwardTsnChunk*)(buf + pos);
        sctp_record_forward_tsn(sctp, ntohl(forward->new_cumulative_tsn));
        SctpSackChunk* sack = (SctpSackChunk*)out_packet->chunks;
        length = sctp_fill_sack(sctp, sack) + sizeof(SctpHeader);
      } break;
      case SCTP_INIT: {
        LOGD("SCTP_INIT");

        if (chunk_length < sizeof(SctpInitChunk)) {
          LOGE("short SCTP INIT chunk");
          return;
        }
        ...
        *(uint32_t*)&param->value = htonl(0x02);
        sctp_write_prsctp_params((uint8_t*)param + 8);
        length = ntohs(init_ack->common.length) + sizeof(SctpHeader);
      } break;
      case SCTP_INIT_ACK: {
        if (chunk_length < sizeof(SctpInitChunk)) {
          LOGE("short SCTP INIT-ACK chunk");
          return;
        }
        ...
        sctp_set_inbound_initial_tsn(sctp, ntohl(init_ack->initial_tsn));
        uint16_t init_length = ntohs(init_ack->common.length);
        uint8_t* params = (uint8_t*)&init_ack->param[0];
        size_t params_length = init_length > 20 ? init_length - 20 : 0;
        for (size_t offset = 0; offset + 4 <= params_length;) {
          SctpChunkParam* candidate = (SctpChunkParam*)(params + offset);
          uint16_t param_length = ntohs(candidate->length);
          if (param_length < 4 || offset + param_length > params_length) {
            break;
          }
          if (ntohs(candidate->type) == SCTP_PARAM_STATE_COOKIE) {
            param = candidate;
            break;
          }
          offset += 4 * ((param_length + 3) / 4);
        }
        if (param == NULL) {
          LOGE("SCTP INIT-ACK missing state cookie");
          return;
        }
...
int sctp_create_association(Sctp* sctp, DtlsSrtp* dtls_srtp) {
  ...
  init_chunk->common.length = htons(20 + 12);
  ...
  sctp_write_prsctp_params((uint8_t*)init_chunk->param);
